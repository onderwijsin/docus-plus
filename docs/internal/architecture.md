...
The layer entry point is [`layer/nuxt.config.ts`](../../layer/nuxt.config.ts). It extends Docus, registers shared
components and integrations, and exposes reusable module configuration. A consuming project extends
the published package from its own Nuxt config; the playground extends the local workspace package
with `extends: ["@onderwijsin/docus-plus"]`.
...
- `playground/` is the feature-complete integration consumer; `.starters/default/` is the full
