# Internal documentation

These notes describe the implementation and maintenance of the reusable `docus-plus` Nuxt layer.
For setup instructions aimed at layer consumers, start with the repository [README](../README.md).

- [Overview](./overview.md)
- [Architecture](./architecture.md)
- [Runtime and configuration](./runtime.md)
- [Development and releases](./development.md)
- [Content and UI conventions](./conventions.md)
- [Local modules](./modules.md)
- [OpenAPI support](./openapi.md)
- [Scalar integration](./scalar.md)
- [Environment files](./environment.md)
- [Agent contract](./agent-contract.md)
